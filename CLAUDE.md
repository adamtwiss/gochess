...
- **Pawn structure** (pawns.go): Cached via pawn hash table. Evaluates doubled, isolated, backward, connected, and passed pawns. Pawn advancement bonus. **Candidate passed pawns**: pawns with no enemy pawn ahead on their own file and enough friendly adjacent pawns to outnumber enemy sentries; bonus by rank via `candidatePassedMG/EG[8]` (~45% of passed pawn base). Gated on `CandidatePassedEnabled`. Precomputed masks: `PassedPawnMask`, `ForwardFileMask`, `OutpostMask`, `AdjacentFiles`.
