...
- **LMP thresholds**: Base 3+depth^2, +50% improving. Could try tighter base or lower improving bonus.
- **Aspiration windows**: Currently delta=15. Small effect expected — low priority.
- **Singular extension margin/depth**: Not eval-dependent but could benefit from tuning.
- **History pruning divisor**: histScore/5000 in LMR adjustments. Could tune.
- **Correction history in LMR**: Use corrected eval for the improving flag calculation.
...
2. **Per-move pruning is more sensitive** (futility, SEE) — errors compound across many moves, so margins need more slack.
3. **NMP and razoring are already well-tuned** — don't revisit unless eval changes significantly.
