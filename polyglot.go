...
//   weight uint16  - frequency/priority
//   learn  uint32  - learning data (ignored)
//
// Entries are sorted by key for binary search.
// Polyglot uses its own Zobrist key scheme with fixed random numbers.

const polyglotEntrySize = 16

// polyglotEntry is a raw entry from a Polyglot .bin file.
...
// OpeningBook provides weighted random move selection from a Polyglot .bin book.
type OpeningBook struct {
	entries map[uint64][]BookMove // Polyglot hash -> moves sorted by weight desc
	rng     *rand.Rand
	useBook bool
}
...
type BookMove struct {
	RawMove uint16 // Polyglot-encoded move (for matching against legal moves)
	Weight  uint16
}

// LoadOpeningBook loads a Polyglot .bin opening book file.
func LoadOpeningBook(filename string) (*OpeningBook, error) {
	...
	if len(data)%polyglotEntrySize != 0 {
		return nil, fmt.Errorf("polyglot book size %d not a multiple of %d", len(data), polyglotEntrySize)
	}

	numEntries := len(data) / polyglotEntrySize

	// Parse all entries and group by key
	grouped := make(map[uint64][]BookMove)

	for i := 0; i < numEntries; i++ {
		offset := i * polyglotEntrySize
		...
		// bytes 12-15 are learn data, ignored

		if weight == 0 {
			continue // skip zero-weight entries
		}

		grouped[key] = append(grouped[key], BookMove{RawMove: move, Weight: weight})
	}

	// Sort each position's moves by weight descending
	for key, moves := range grouped {
...
// PickMove selects a weighted random move for the given board position.
// It computes the Polyglot hash, looks up book moves, and matches them
// against legal moves to handle en passant detection and castling conversion.
// Returns (move, found).
func (book *OpeningBook) PickMove(b *Board) (Move, bool) {
	if !book.useBook {
		return NoMove, false
	}
	...
	// Generate legal moves for matching
	legalMoves := b.GenerateLegalMoves()

	// Sum weights (only for moves that match a legal move)
...
// matchPolyglotMove finds the legal move that corresponds to a Polyglot-encoded move.
// Polyglot encodes castling as king-to-rook (e.g. e1h1) but we use king-to-destination (e1g1).
// En passant flags can't be determined from the Polyglot encoding alone, so we match by from/to/promo.
func matchPolyglotMove(pm uint16, legalMoves []Move, b *Board) (Move, bool) {
	toFile := int(pm & 7)
	toRank := int((pm >> 3) & 7)
	fromFile := int((pm >> 6) & 7)
	fromRank := int((pm >> 9) & 7)
	promoPiece := int((pm >> 12) & 7)

	from := Square(fromRank*8 + fromFile)
	to := Square(toRank*8 + toFile)

	// Convert Polyglot castling (king-to-rook) to king-to-destination
	if fromFile == 4 { // king on e-file
		if fromRank == 0 && toRank == 0 { // rank 1 (white)
			if toFile == 7 { // e1h1 -> e1g1
				to = Square(6)
			} else if toFile == 0 { // e1a1 -> e1c1
				to = Square(2)
			}
		} else if fromRank == 7 && toRank == 7 { // rank 8 (black)
			if toFile == 7 { // e8h8 -> e8g8
				to = Square(62)
			} else if toFile == 0 { // e8a8 -> e8c8
				to = Square(58)
			}
		}
	}

	// Determine expected promotion flag
	var promoFlag int
	switch promoPiece {
	case 1:
...
// PolyglotHash computes the Polyglot Zobrist hash for a board position.
func PolyglotHash(b *Board) uint64 {
	var hash uint64
	...
	for sq := 0; sq < 64; sq++ {
		...
		hash ^= polyglotRandoms[idx*64+sq]
	}

	// Castling
	if b.Castling&WhiteKingside != 0 {
		hash ^= polyglotRandoms[polyglotCastleOffset]
	}
	if b.Castling&WhiteQueenside != 0 {
		hash ^= polyglotRandoms[polyglotCastleOffset+1]
	}
	if b.Castling&BlackKingside != 0 {
		hash ^= polyglotRandoms[polyglotCastleOffset+2]
	}
	if b.Castling&BlackQueenside != 0 {
		hash ^= polyglotRandoms[polyglotCastleOffset+3]
	}

	// En passant (only if a pawn can actually capture)
	if b.EnPassant != NoSquare {
		epFile := b.EnPassant.File()
		if polyglotHasEPCapture(b) {
			hash ^= polyglotRandoms[polyglotEPOffset+epFile]
		}
	}

	// Side to move: Polyglot XORs the turn key when it's WHITE's turn
	if b.SideToMove == White {
		hash ^= polyglotRandoms[polyglotTurnOffset]
	}

	return hash
}

// polyglotHasEPCapture checks if any pawn can actually make the en passant capture.
func polyglotHasEPCapture(b *Board) bool {
	epSq := b.EnPassant
	epFile := epSq.File()
	if b.SideToMove == White {
		rank4 := 4 // rank 5 is index 4 in 0-based
		if epFile > 0 {
			sq := Square(rank4*8 + epFile - 1)
			if b.Squares[sq] == WhitePawn {
				return true
			}
		}
		if epFile < 7 {
		...
		rank3 := 3
		if epFile > 0 {
			sq := Square(rank3*8 + epFile - 1)
			if b.Squares[sq] == BlackPawn {
				return true
			}
		}
		if epFile < 7 {
...
// polyglotPieceIndex maps our piece encoding to Polyglot's.
// Polyglot: BlackPawn=0, WhitePawn=1, BlackKnight=2, WhiteKnight=3, ...
func polyglotPieceIndex(p Piece) int {
	switch p {
	case BlackPawn:
		return 0
	case WhitePawn:
...
// buildPolyglotBook creates an OpeningBook directly from a map of Polyglot hash -> moves.
// Used for testing.
func buildPolyglotBook(entries map[uint64][]BookMove) *OpeningBook {
	return &OpeningBook{
		entries: entries,
...
// polyglotRandoms contains the 781 random numbers defined by the Polyglot specification.
var polyglotRandoms = [781]uint64{
