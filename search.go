		...
		if depth >= 4 && prevScore > -MateScore+100 && prevScore < MateScore-100 {
			...
			for {
				...
				if score >= beta {
					...
					// Contract alpha toward beta, widen beta based on score.
...
func (b *Board) negamax(depth, ply int, alpha, beta int, info *SearchInfo) int {
	...
	for {
		...
		_ = singularExtension // singular extensions currently disabled
	...
	// Formula: (3 + depth*depth), +50% when improving
		...
		// SPRT-validated: recapture is essential (-18.2 Elo when disabled).
