...
// TestSEEQuietPruneCorrectness verifies SEE quiet pruning doesn't break tactical correctness
func TestSEEQuietPruneCorrectness(t *testing.T) {
	positions := []struct {
		name     string
		fen      string
		best     string
	...
	SEEQuietPruneEnabled = true
	defer func() { SEEQuietPruneEnabled = true }()
	...
	for _, pos := range positions {
		t.Run(pos.name, func(t *testing.T) {
			...
			t.Logf("Move: %s, Score: %d, Nodes: %d, SEE quiet prunes: %d",
				move, info.Score, info.Nodes, info.SEEQuietPrunes)
		})
	}
}

// TestSEEQuietPruneComparison compares search with and without SEE quiet pruning
func TestSEEQuietPruneComparison(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping SEE quiet prune comparison in short mode")
	}
	...
	t.Log("=== SEE Quiet Prune Comparison ===")
	t.Logf("Testing at depth %d\n", depth)
	...
	for _, pos := range positions {
		...
		// Without SEE quiet pruning
		SEEQuietPruneEnabled = false
		tt2 := NewTranspositionTable(16)
		start = time.Now()
		moveWithout, infoWithout := board.SearchWithTT(depth, 0, tt2)
		timeWithout := time.Since(start)
		...
		t.Logf("\n%s:", pos.name)
		t.Logf("  With SEE prune:    %s, nodes=%d, time=%v, SEE prunes=%d",
			moveWith, infoWith.Nodes, timeWith, infoWith.SEEQuietPrunes)
		t.Logf("  Without SEE prune: %s, nodes=%d, time=%v",
			moveWithout, infoWithout.Nodes, timeWithout)
		t.Logf("  Node reduction: %.1f%%, Speedup: %.2fx", nodeReduction, speedup)

		if moveWith != moveWithout {
			t.Logf("  NOTE: Different moves found (SEE prune effect)")
		}
	}
	...
	t.Logf("\n=== TOTALS ===")
	t.Logf("Total nodes with SEE prune:    %d", totalNodesWith)
